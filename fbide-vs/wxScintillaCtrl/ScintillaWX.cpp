...
//
// Created:     13-Jan-2000
// RCS-ID:      $Id: ScintillaWX.cpp 62075 2009-09-24 13:17:14Z JS $
...
ScintillaWX::ScintillaWX(wxScintillaCtrl* win) {
    capturedMouse = false;
    focusEvent = false;
    wMain = win;
    stc   = win;
...
bool ScintillaWX::SetIdle(bool on) {
    if (idler.state != on) {
        ...
        if (on)
            stc->Connect(wxID_ANY, wxEVT_IDLE, wxIdleEventHandler(wxScintillaCtrl::OnIdle));
        else
            stc->Disconnect(wxID_ANY, wxEVT_IDLE, wxIdleEventHandler(wxScintillaCtrl::OnIdle));
        idler.state = on;
    }
    return idler.state;
}
...
sptr_t ScintillaWX::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
      ...
      case SCI_CALLTIPSHOW: {
          ...
          PRectangle rc = ct.CallTipStart(CurrentPosition(), pt,
                                          defn,
...
wxDragResult ScintillaWX::DoDragOver(wxCoord x, wxCoord y, wxDragResult def) {
    SelectionPosition selpos(PositionFromLocation(Point(x, y)));
    SetDragPosition(selpos);
